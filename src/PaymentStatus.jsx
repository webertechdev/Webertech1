  ...
  if (step === "paid") {
    return (
      <Centered>
        ...
        <p style={p}>{product?.title} is unlocked. Download the original file below; it has no preview watermark.</p>
        {downloadUrl ? (
          <a
            href={downloadUrl}
            download={product?.fileName || product?.title || "webertech-document.pdf"}
            style={primaryBtn}
          >
            ⬇ Download original PDF
          </a>
        ) : (
          <p style={{ ...p, color: "#b45309" }}>Your secure download link is still being prepared. Close this message and refresh once.</p>
        )}
