// src/pages/UnifiedControlCenter.jsx
// WeberTech Unified Control Center
// All admin functions in one powerful dashboard: Documents, AI, Support, Analytics
...
export default function UnifiedControlCenter() {
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      const aiSnap = await getDocs(collection(db, "config")).catch(() => ({ docs: [] }));
      const aiConfig = aiSnap.docs.find(d => d.id === "weberai");
      if (aiConfig?.exists()) {
        setAiTraining(aiConfig.data());
      }
  ...
  const handleSaveAITraining = async () => {
    ...
    try {
      ...
      toast.error("Failed to save AI training");
    }
    setLoading(false);
  };
  ...
  return (
    <>
      <style>{`
        ...
        .uc-btn-secondary { background: rgba(255,255,255,0.1); color: #fff; }
        .uc-btn-secondary:hover { background: rgba(255,255,255,0.15); }
          ...
          <main className="uc-main">
            ...
            {tab === "ai" && (
              <div className="uc-card">
                ...
                <textarea
                  className="uc-textarea"
                  placeholder="Paste all service details, pricing, and product information here..."
                ...
                <textarea
                  className="uc-textarea"
                  placeholder="How should the AI handle different scenarios? E.g., 'Always ask for phone number before providing services.'"
            ...
            {tab === "support" && (
              <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 16 }}>
                <div className="uc-card" style={{ maxHeight: "600px", overflowY: "auto" }}>
                  <h3 className="uc-card-title">💬 Active Chats</h3>
                  {chats.length === 0 ? (
