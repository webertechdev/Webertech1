...
export default function LegalDocumentDetail() {
  ...
  const closePayment = () => {
    if (paymentState.step !== "starting" && paymentState.step !== "awaiting") {
      // Preserve the paid order so the original download remains available
      // after the success dialog is closed. Failed attempts can be reset.
      if (paymentState.step !== "paid") reset();
